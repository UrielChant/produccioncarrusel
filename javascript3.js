url_endpoint='https://carruselmock.free.beeceptor.com/'; 
var temporal="";
var identidad="";
...
async function refrescar() {
    ...
    try {
        ...
        console.log(total,page,page_size)
        console.log("esperar");
        if (response.ok) {
            ...
            var conteo=0;
                ...
                deleteButton.addEventListener("click", () => borraranuncio(item.name));//aqui se coloca el identificador  para realizar supresion
                row.insertCell(6).appendChild(deleteButton);

                const viewButton = document.createElement("button");
                viewButton.textContent = "Visualizar";
                viewButton.addEventListener("click", () => veranuncio(item.name)); //aqui se coloca el identificador  para realizar lectura
                row.insertCell(7).appendChild(viewButton);
...
async function veranuncio(itemId) {
    ...
    if (response.ok) {
        ...
        row2.insertCell(6).appendChild(deleteButton);
        var row2 = tableBody.insertRow();
